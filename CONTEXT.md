...
| Datei | Verantwortung |
|---|---|
| `api_client.py` | **DER eine Auth-Pfad** (ab v3.27.0): `CrowdergyAuthSession` (Token-Paar, Single-Flight-Refresh mit CAS, 401-retry-once, lazy Executor-Client), `authenticated_request` (re-used die Coordinator-Session wenn vorhanden), `claim_pairing_code` (kanonisch `/connector/claim`, Fallback `/box/claim`), `fetch_account_email` |
...
- httpx-Clients werden IMMER im Executor gebaut (synchroner CA-Load →
  HA-Blocking-Call-Warnung, live gefunden); ab v3.27.0 zentral in
...
`test_api_client` (Single-Flight/CAS, 401-retry-once,
Claim-Fallback-Vertrag), `test_config_flow_pairing` (user-Step + Reauth
inkl. Account-Mismatch), `test_sse_client`, `test_state_mirror`,
