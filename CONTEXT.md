...
| `telemetry_reader.py` | Per-Tick-Entity-Reader (`_read_power_kw`, `_read_energy_kwh`, …), die Send-Entscheidung `_should_send` (Schwellen + Soft-/Hard-Heartbeat; `is_online` zählt seit #151 zu den kategorischen Feldern) und `_measurement_liveness(dev)` — **misst dieses Gerät noch?** Sind Mess-Slots gemappt und liefert KEINER einen Wert (alle-oder-keiner über `_MEASUREMENT_SLOT_KEYS` + die Typ-Extras; Steuer-Slots zählen bewusst NICHT mit), sendet der Tick `is_online: false` und lässt `power_kw` WEG statt es zu nullen (#151, v3.47.0). |
